...
class SMCDataPipeline:
    ...
    def detect_order_blocks(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(3, len(df) - 3):
            ...
            if (df['close'].iloc[i-1] < df['open'].iloc[i-1] and  # Bearish candle (color change)
                ...
                body_size_atr = abs(df['close'].iloc[i-1] - df['open'].iloc[i-1]) / atr
                
                # Fuzzy logic classification
                # Classify displacement strength
                disp_fuzzy = self.fuzzy_classifier.classify_displacement(displacement_atr)
                disp_score = disp_fuzzy['moderate'] + disp_fuzzy['strong'] + disp_fuzzy['extreme']
                
                # Classify candle body
                body_fuzzy = self.fuzzy_classifier.classify_candle_body(body_size_atr)
                body_score = body_fuzzy['small'] + body_fuzzy['medium'] + body_fuzzy['large']
                
                # Fuzzy AND for overall quality (minimum of both scores)
                quality_score = self.fuzzy_classifier.fuzzy_and(disp_score, body_score)
                ...
                if quality_score >= self.fuzzy_quality_threshold:
                    ...
                    df.at[i-1, 'OB_Size_ATR'] = (df['high'].iloc[i-1] - df['low'].iloc[i-1]) / atr
        ...
        # Track OB age and mitigation status
        for i in range(len(df)):
            if df['OB_Bullish'].iloc[i] == 1 or df['OB_Bearish'].iloc[i] == 1:
                ...
                for j in range(i+1, len(df)):
                    ...
                    # Bullish OB mitigated when price trades below OB low
                    if is_bullish and df['low'].iloc[j] <= ob_low:
                        df.at[ob_index, 'OB_Mitigated'] = 1
                        break
                    
                    # Bearish OB mitigated when price trades above OB high
                    if not is_bullish and df['high'].iloc[j] >= ob_high:
                        df.at[ob_index, 'OB_Mitigated'] = 1
                        break
    ...
    def detect_fair_value_gaps(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(2, len(df)):
            ...
            if c1_high < c3_low:
                ...
                gap_fuzzy = self.fuzzy_classifier.classify_gap_size(gap_depth_atr)
    ...
    def detect_market_structure(self, df: pd.DataFrame, swing_window: int = 10) -> pd.DataFrame:
        ...
        for i in range(swing_window, len(df) - swing_window):
            ...
            if df['high'].iloc[i] > recent_high:
                ...
                df.at[i, 'BOS_Dist_ATR'] = (df['high'].iloc[i] - recent_high) / atr
                
                # Close break indicates stronger commitment
                if df['close'].iloc[i] > recent_high:
                    df.at[i, 'BOS_Close_Confirm'] = 1
                    df.at[i, 'BOS_Commitment_Flag'] = 1  # High-conviction signal
            ...
            if df['low'].iloc[i] < recent_low:
                df.at[i, 'BOS_Wick_Confirm'] = -1
                df.at[i, 'BOS_Dist_ATR'] = (recent_low - df['low'].iloc[i]) / atr
    ...
    def add_regime_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['Trend_Bias_Indicator'] = (df['close'] - df['EMA_50']) / (df['atr'] + 1e-10)
        ...
        df['ATR_ZScore'] = (df['atr'] - df['ATR_MA']) / (df['atr'].rolling(50).std() + 1e-10)
        ...
        for i in range(len(df)):
            if pd.notna(df['ATR_ZScore'].iloc[i]) and pd.notna(df['Trend_Strength_Fuzzy'].iloc[i]):
                ...
                high_vol_membership = max(0, min(1, (atr_z - 0.5) / 1.0))  # Gradual transition
                low_vol_membership = max(0, min(1, (-atr_z + 0.5) / 1.5))
                strong_trend_membership = max(0, min(1, (trend_fuzzy - 1.0) / 1.0))
                weak_trend_membership = max(0, min(1, (1.0 - trend_fuzzy) / 0.5))
    ...
    def apply_regime_filters(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        bullish_trend_membership = df.apply(
            lambda row: max(0, min(1, (row['close'] - row['EMA_50']) / (row['atr'] + 1e-10))) 
            if pd.notna(row['close']) and pd.notna(row['EMA_50']) and pd.notna(row['atr']) else 0,
            axis=1
        )
        bearish_trend_membership = df.apply(
            lambda row: max(0, min(1, (row['EMA_50'] - row['close']) / (row['atr'] + 1e-10))) 
            if pd.notna(row['close']) and pd.notna(row['EMA_50']) and pd.notna(row['atr']) else 0,
            axis=1
        )
        
        # Fuzzy momentum filter
        overbought_membership = df['RSI'].apply(lambda x: max(0, min(1, (x - 60) / 20)) if pd.notna(x) else 0)
        oversold_membership = df['RSI'].apply(lambda x: max(0, min(1, (40 - x) / 20)) if pd.notna(x) else 0)
        
        # Fuzzy regime filter
        trending_regime = df['Volatility_Regime_Fuzzy'].isin(['High_Vol_Trend', 'Normal'])
    ...
    def apply_triple_barrier_method(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(df) - self.lookforward):
            # Check if this is a valid entry point (OB or FVG retest)
            is_bullish_entry = (df['OB_Bullish_Valid'].iloc[i] or df['FVG_Bullish_Valid'].iloc[i])
            is_bearish_entry = (df['OB_Bearish_Valid'].iloc[i] or df['FVG_Bearish_Valid'].iloc[i])
            ...
            if is_bullish_entry:
                ...
                if df['OB_Bullish_Valid'].iloc[i]:
                    stop_loss = df['OB_Low'].iloc[i] - (0.1 * atr)  # Margin for slippage
                else:
                ...
                if df['OB_Bearish_Valid'].iloc[i]:
                    stop_loss = df['OB_High'].iloc[i] + (0.1 * atr)  # Margin for slippage
                else:
    ...
    def normalize_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for feature in features_to_standardize:
            if feature in df.columns:
                ...
                df[f'{feature}_ZScore'] = (df[feature] - rolling_mean) / (rolling_std + 1e-10)
        ...
        for i in range(len(df)):
            ...
            if df['OB_Bullish'].iloc[i] == 1 and pd.notna(df['OB_Low'].iloc[i]):
                ...
                if pd.notna(df['atr'].iloc[i]) and df['atr'].iloc[i] > 0:
                    df.at[i, 'Distance_to_Entry_ATR'] = distance / df['atr'].iloc[i]
    ...
    def add_multi_timeframe_confluence(self, base_df: pd.DataFrame, all_data: pd.DataFrame, 
        ...
        # Initialize confluence columns with fuzzy features
        base_df['HTF_OB_Confluence'] = 0
        base_df['HTF_OB_Proximity_Fuzzy'] = 0.0
        base_df['HTF_FVG_Confluence'] = 0
        base_df['HTF_FVG_Proximity_Fuzzy'] = 0.0
        base_df['HTF_Trend_Alignment'] = 0
        base_df['HTF_Structure_Alignment'] = 0
        base_df['HTF_Confluence_Quality'] = 0.0
        
        # Process each higher timeframe
        for htf in self.higher_tfs:
            # Get higher timeframe data
            htf_mask = (all_data['symbol'] == symbol) & (all_data['timeframe'] == htf)
            ...
            for i in range(len(base_df)):
                ...
                # Track fuzzy scores for this HTF
                htf_ob_fuzzy_scores = []
                htf_fvg_fuzzy_scores = []
                
                # Check OB confluence with fuzzy proximity
                if 'OB_Bullish' in htf_df.columns and 'OB_Bearish' in htf_df.columns:
                    # Look back in HTF for recent unmitigated OBs (increased from 20 to 50)
                    lookback_start = max(0, htf_current_idx - 50)
                    htf_recent = htf_df.iloc[lookback_start:htf_current_idx+1]
                    
                    # Check if current price is near HTF OB
                    for j in range(len(htf_recent)):
                        # Skip mitigated OBs
                        if htf_recent['OB_Mitigated'].iloc[j] == 1:
                            continue
                        
                        if htf_recent['OB_Bullish'].iloc[j] == 1 or htf_recent['OB_Bearish'].iloc[j] == 1:
                            ob_low = htf_recent['OB_Low'].iloc[j]
                            ob_high = htf_recent['OB_High'].iloc[j]
                            
                            if pd.notna(ob_low) and pd.notna(ob_high):
                                # Calculate fuzzy proximity using triangular membership
                                # Center of OB
                                ob_center = (ob_high + ob_low) / 2
                                ob_size = ob_high - ob_low
                                
                                # Distance from price to OB center
                                distance = abs(base_price - ob_center)
                                distance_atr = distance / base_atr
                                
                                # Fuzzy membership: 1.0 at center, decays to 0 at 5 ATR away (increased from 3)
                                # Using triangular function: peak at center, zero at boundaries
                                proximity_fuzzy = self.fuzzy_classifier.mf.triangular(
                                    distance_atr, 
                                    a=0.0,      # Start of influence
                                    b=0.0,      # Peak at center
                                    c=5.0       # End of influence at 5 ATR (was 3.0)
                                )
                                
                                # Weight by HTF OB quality if available
                                if 'OB_Quality_Fuzzy' in htf_recent.columns:
                                    htf_ob_quality = htf_recent['OB_Quality_Fuzzy'].iloc[j]
                                    if pd.notna(htf_ob_quality):
                                        proximity_fuzzy = self.fuzzy_classifier.fuzzy_and(
                                            proximity_fuzzy, htf_ob_quality
                                        )
                                
                                if proximity_fuzzy > 0.05:  # Threshold for relevance (lowered from 0.1)
                                    htf_ob_fuzzy_scores.append(proximity_fuzzy)
                                    
                                    # Binary confluence count (for backward compatibility)
                                    if ob_low <= base_price <= ob_high:
                                        base_df.at[i, 'HTF_OB_Confluence'] += 1
                
                # Check FVG confluence with fuzzy proximity
                if 'FVG_Bullish' in htf_df.columns and 'FVG_Bearish' in htf_df.columns:
                    lookback_start = max(0, htf_current_idx - 50)  # Increased from 20 to 50
                    htf_recent = htf_df.iloc[lookback_start:htf_current_idx+1]
                    
                    for j in range(len(htf_recent)):
                        # Skip mitigated FVGs
                        if htf_recent['FVG_Mitigated'].iloc[j] == 1:
                            continue
                        
                        if htf_recent['FVG_Bullish'].iloc[j] == 1 or htf_recent['FVG_Bearish'].iloc[j] == 1:
                            fvg_bottom = htf_recent['FVG_Bottom'].iloc[j]
                            fvg_top = htf_recent['FVG_Top'].iloc[j]
                            
                            if pd.notna(fvg_bottom) and pd.notna(fvg_top):
                                # Calculate fuzzy proximity to FVG
                                fvg_center = (fvg_top + fvg_bottom) / 2
                                fvg_size = fvg_top - fvg_bottom
                                
                                # Distance from price to FVG center
                                distance = abs(base_price - fvg_center)
                                distance_atr = distance / base_atr
                                ...
                                proximity_fuzzy = self.fuzzy_classifier.mf.triangular(
                                    ...
                                    a=0.0,
                                    b=0.0,
                                    c=5.0  # Increased from 3.0 to 5.0
                                )
                                
                                # Weight by HTF FVG quality if available
                                if 'FVG_Quality_Fuzzy' in htf_recent.columns:
                                    htf_fvg_quality = htf_recent['FVG_Quality_Fuzzy'].iloc[j]
                                    if pd.notna(htf_fvg_quality):
                                        proximity_fuzzy = self.fuzzy_classifier.fuzzy_and(
                                            proximity_fuzzy, htf_fvg_quality
                                        )
                                
                                if proximity_fuzzy > 0.05:  # Lowered from 0.1
                                    htf_fvg_fuzzy_scores.append(proximity_fuzzy)
                                    
                                    # Binary confluence count
                                    if fvg_bottom <= base_price <= fvg_top:
                                        base_df.at[i, 'HTF_FVG_Confluence'] += 1
                
                # Aggregate fuzzy scores using fuzzy OR (maximum)
                if htf_ob_fuzzy_scores:
                    max_ob_fuzzy = max(htf_ob_fuzzy_scores)
                    base_df.at[i, 'HTF_OB_Proximity_Fuzzy'] = max(
                        base_df.at[i, 'HTF_OB_Proximity_Fuzzy'], max_ob_fuzzy
                    )
                
                if htf_fvg_fuzzy_scores:
                    max_fvg_fuzzy = max(htf_fvg_fuzzy_scores)
                    base_df.at[i, 'HTF_FVG_Proximity_Fuzzy'] = max(
                        base_df.at[i, 'HTF_FVG_Proximity_Fuzzy'], max_fvg_fuzzy
                    )
                
                # Check trend alignment with fuzzy strength assessment
                if 'EMA_50' in htf_df.columns and 'EMA_50' in base_df.columns:
                    htf_close = htf_df.loc[htf_current_idx, 'close']
                    htf_ema50 = htf_df.loc[htf_current_idx, 'EMA_50']
                    base_close = base_df['close'].iloc[i]
                    base_ema50 = base_df['EMA_50'].iloc[i]
                    
                    if all(pd.notna(x) for x in [htf_close, htf_ema50, base_close, base_ema50]):
                        # Calculate trend strength using distance from EMA
                        htf_trend_dist = abs(htf_close - htf_ema50) / base_atr
                        base_trend_dist = abs(base_close - base_ema50) / base_atr
                        
                        # Classify trend strength using fuzzy logic
                        htf_trend_fuzzy = self.fuzzy_classifier.classify_trend_strength(htf_trend_dist)
                        base_trend_fuzzy = self.fuzzy_classifier.classify_trend_strength(base_trend_dist)
                        
                        # HTF bullish trend aligned with base bullish trend
                        if htf_close > htf_ema50 and base_close > base_ema50:
                            # Fuzzy AND of both trend strengths
                            trend_strength = self.fuzzy_classifier.fuzzy_and(
                                htf_trend_fuzzy['moderate'] + htf_trend_fuzzy['strong'],
                                base_trend_fuzzy['moderate'] + base_trend_fuzzy['strong']
                            )
                            if trend_strength > 0.3:
                                base_df.at[i, 'HTF_Trend_Alignment'] = 1
                        
                        # HTF bearish trend aligned with base bearish trend
                        elif htf_close < htf_ema50 and base_close < base_ema50:
                            trend_strength = self.fuzzy_classifier.fuzzy_and(
                                htf_trend_fuzzy['moderate'] + htf_trend_fuzzy['strong'],
                                base_trend_fuzzy['moderate'] + base_trend_fuzzy['strong']
                            )
                ...
                # Check structure alignment (BOS) with commitment weighting
                if 'BOS_Close_Confirm' in htf_df.columns:
                    htf_bos = htf_df.loc[htf_current_idx, 'BOS_Close_Confirm']
                    if htf_bos != 0:
                        # Weight by commitment flag if available
                        commitment_weight = 1.0
                        if 'BOS_Commitment_Flag' in htf_df.columns:
                            if htf_df.loc[htf_current_idx, 'BOS_Commitment_Flag'] == 1:
                                commitment_weight = 1.5  # Higher weight for close breaks
                        
                        base_df.at[i, 'HTF_Structure_Alignment'] = htf_bos * commitment_weight
        
        # Calculate overall confluence quality using fuzzy aggregation
        for i in range(len(base_df)):
            # Combine all confluence factors using fuzzy OR (maximum)
            ob_score = base_df['HTF_OB_Proximity_Fuzzy'].iloc[i]
            fvg_score = base_df['HTF_FVG_Proximity_Fuzzy'].iloc[i]
            
            # Normalize trend alignment to [0, 1]
            trend_score = abs(base_df['HTF_Trend_Alignment'].iloc[i]) * 0.5
            
            # Normalize structure alignment to [0, 1]
            structure_score = min(abs(base_df['HTF_Structure_Alignment'].iloc[i]) / 1.5, 1.0)
            
            # Fuzzy OR aggregation (maximum of all scores)
            confluence_quality = max(ob_score, fvg_score, trend_score, structure_score)
            
            # Apply fuzzy AND with base timeframe structure quality if available
            if 'OB_Quality_Fuzzy' in base_df.columns:
                base_ob_quality = base_df['OB_Quality_Fuzzy'].iloc[i]
                if pd.notna(base_ob_quality) and base_ob_quality > 0:
                    confluence_quality = self.fuzzy_classifier.fuzzy_and(
                        confluence_quality, base_ob_quality
                    )
            
            base_df.at[i, 'HTF_Confluence_Quality'] = confluence_quality
    ...
    def process_symbol_timeframe(self, df: pd.DataFrame, symbol: str, timeframe: str, all_data: pd.DataFrame = None) -> pd.DataFrame:
        ...
        if len(symbol_df) < 100:
            print(f"    ⚠️ Insufficient data ({len(symbol_df)} rows), skipping")
            return pd.DataFrame()
        ...
        if all_data is not None and timeframe == self.base_tf:
            symbol_df = self.add_multi_timeframe_confluence(symbol_df, all_data, symbol, timeframe)
        ...
        print(f"    ✓ Valid entries: {valid_entries}, Labeled: {labeled_entries}")
    ...
    def process_all_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for symbol in df['symbol'].unique():
            for timeframe in df['timeframe'].unique():
                processed_df = self.process_symbol_timeframe(df, symbol, timeframe, all_data=None)
        ...
        for symbol in combined_df['symbol'].unique():
            ...
            base_mask = (combined_df['symbol'] == symbol) & (combined_df['timeframe'] == self.base_tf)
            base_df = combined_df[base_mask].copy()
            
            if not base_df.empty:
                # Add confluence features
                base_df = self.add_multi_timeframe_confluence(base_df, combined_df, symbol, self.base_tf)
                final_dfs.append(base_df)
            
            # Also include higher timeframe data (without confluence)
            for htf in self.higher_tfs:
                htf_mask = (combined_df['symbol'] == symbol) & (combined_df['timeframe'] == htf)
        ...
        if not final_dfs:
            raise ValueError("No data with confluence was successfully processed!")
    ...
    def generate_summary_report(self, df: pd.DataFrame) -> Dict:
        ...
        print(f"  BOS Close Breaks (High Conviction): {df['BOS_Close_Confirm'].abs().sum():,}")
        ...
        if 'HTF_OB_Confluence' in df.columns:
            ...
            htf_fvg_conf = df[df['HTF_FVG_Confluence'] > 0]['HTF_FVG_Confluence']
            ...
            print(f"  Setups with HTF Trend Alignment: {len(htf_trend_align):,}")
        ...
        if labeled_mask.sum() > 0:
            ...
            label_counts = df.loc[labeled_mask, 'TBM_Label'].value_counts().sort_index()
            total_labeled = labeled_mask.sum()
            
            for label, count in label_counts.items():
                label_name = {-1: 'Loss', 0: 'Timeout', 1: 'Win'}.get(label, 'Unknown')
                pct = (count / total_labeled) * 100
                print(f"    {label_name:8s}: {count:6,} ({pct:5.1f}%)")
            
            # Win rate (excluding timeouts)
            decisive = df.loc[labeled_mask & (df['TBM_Label'] != 0), 'TBM_Label']
            if len(decisive) > 0:
                ...
                if 'TBM_Bars_to_Hit' in df.columns:
                    avg_bars = df.loc[labeled_mask & df['TBM_Bars_to_Hit'].notna(), 'TBM_Bars_to_Hit'].mean()
    ...
    def split_data_for_ml(self, df: pd.DataFrame, train_ratio: float = 0.7, 
        ...
        for symbol in df['symbol'].unique():
            ...
            base_tf_df = symbol_df[symbol_df['timeframe'] == self.base_tf].copy()
        ...
        for split_name in ['train', 'val', 'test']:
            if split_name in splits and splits[split_name]:
                final_splits[split_name] = pd.concat(splits[split_name], ignore_index=True)
        ...
        for symbol in df['symbol'].unique():
            ...
            if stratify_by_quality and 'OB_Quality_Fuzzy' in base_tf_df.columns:
                ...
                for stratum in ['Low', 'Medium', 'High']:
                    ...
                    strata_splits['val'].append(stratum_df.iloc[train_end:val_end])
                    strata_splits['test'].append(stratum_df.iloc[val_end:])
                
                # Combine strata and sort by time
                for split_name in ['train', 'val', 'test']:
                    if strata_splits[split_name]:
                        split_data = pd.concat(strata_splits[split_name]).sort_values('time')
    ...
    def create_time_series_cv_folds(self, df: pd.DataFrame, n_splits: int = 5,
        ...
        base_df = df[df['timeframe'] == self.base_tf].copy().sort_values('time')
        
        if len(base_df) < n_splits * 100:
            raise ValueError(f"Insufficient data for {n_splits} folds. Need at least {n_splits * 100} samples.")
    ...
    def validate_split_quality_distribution(self, splits: Dict[str, pd.DataFrame]) -> Dict[str, Dict]:
        ...
        quality_cols = ['OB_Quality_Fuzzy', 'FVG_Quality_Fuzzy', 'HTF_Confluence_Quality']
        available_quality_cols = [col for col in quality_cols if col in splits['train'].columns]
        ...
        for col in available_quality_cols:
            print(f"\n  {col}:")
            print(f"    {'Split':<8} {'Mean':<8} {'Std':<8} {'Q25':<8} {'Q50':<8} {'Q75':<8}")
            print(f"    {'-'*56}")
        ...
        if 'train' in stats and 'test' in stats:
            for col in available_quality_cols:
                if col in stats['train'] and col in stats['test']:
                    ...
                    print(f"    {status} {col}: Train-Test difference = {diff_pct:.1f}%")
    ...
    def save_processed_data(self, df: pd.DataFrame, output_path: str, 
        ...
        print(f"✓ Saved full dataset: {len(df):,} rows ({file_size_mb:.2f} MB)")
        
        # Save train/val/test splits
        if save_splits:
            # Use fuzzy stratification if available
            if use_fuzzy_stratification and 'OB_Quality_Fuzzy' in df.columns:
                splits = self.split_data_with_fuzzy_stratification(df, *split_ratios)
            ...
            for split_name, split_df in splits.items():
                if not split_df.empty:
                    ...
                    print(f"✓ Saved {split_name} split: {len(split_df):,} rows ({split_size_mb:.2f} MB)")
            
            # Validate quality distribution
            if use_fuzzy_stratification:
                quality_stats = self.validate_split_quality_distribution(splits)
                ...
                with open(stats_path, 'w') as f:
                    ...
                    f.write("# Validates balanced distribution across train/val/test splits\n\n")
            ...
            if save_cv_folds:
                try:
                    cv_folds = self.create_time_series_cv_folds(df, n_splits=n_cv_folds)
    ...
    def run_pipeline(self, input_path: str, output_path: str, 
        ...
        self.save_processed_data(processed_df, output_path, save_splits=save_splits)
